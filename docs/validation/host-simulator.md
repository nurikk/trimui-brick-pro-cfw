...
test "$(docker ps --filter label=org.trimui-brick-pro-cfw.simulator -q)" = ""
...
docker run --rm --network none \
  ...
  -w /workspace \
  --entrypoint cargo \
  trimui-brick-pro-cfw-simulator:local \
  fmt --check
...
CONTAINER=$(docker ps --filter label=org.trimui-brick-pro-cfw.simulator -q)
...
The image build uses `containers/simulator/Dockerfile` and is based on the immutable x86_64 digest and records the toolchain and direct package versions in `sim/container-versions.txt`; `Cargo.lock` records all Rust dependency versions.
